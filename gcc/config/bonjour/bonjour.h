/* Definitions of target machine for GNU compiler, for BONJOUR.
...
#define BONJOUR_UNITS_PER_DWORD 4
...
/* In BONJOUR arrays of chars are word-aligned, so strcpy () will be faster.  */
...
/* In BONJOUR strings are word-aligned; strcpy from constants will be faster.  */
...
#define BONJOUR_FIRST_DWORD_REGISTER   12

#define FIRST_PSEUDO_REGISTER       16

/* 1 for registers that have pervasive standard uses
   and are not available for the register allocator.
   On the BONJOUR, only the stack pointer (r15) is such.  */
...
#define HARD_REGNO_NREGS(REGNO, MODE)                                         \
 ((REGNO >= BONJOUR_FIRST_DWORD_REGISTER)                                        \
  ? ((GET_MODE_SIZE (MODE) + BONJOUR_UNITS_PER_DWORD - 1) / BONJOUR_UNITS_PER_DWORD)\
...
#define BONJOUR_REGNO_OK_FOR_BASE_P(REGNO)                  \
...
#define REGNO_MODE_OK_FOR_BASE_P(REGNO, MODE)	  \
	(BONJOUR_REGNO_OK_FOR_BASE_P(REGNO)  &&	  \
...
#define REGNO_OK_FOR_INDEX_P(REGNO)        \
  (((REGNO >= BONJOUR_FIRST_DWORD_REGISTER)   \
     && ((REGNO) < FIRST_PSEUDO_REGISTER)) \
   || (reg_renumber                        \
       && (((unsigned) reg_renumber[REGNO] >= BONJOUR_FIRST_DWORD_REGISTER)  \
...
#define INITIAL_ELIMINATION_OFFSET(FROM, TO, OFFSET)              \
  do                                                              \
    {                                                             \
      (OFFSET) = bonjour_initial_elimination_offset ((FROM), (TO));  \
    }                                                             \
  while (0)
...
#define INIT_CUMULATIVE_ARGS(CUM, FNTYPE, LIBNAME, FNDECL, N_NAMED_ARGS) \
  bonjour_init_cumulative_args (&(CUM), (FNTYPE), (LIBNAME))

#define FUNCTION_ARG_REGNO_P(REGNO)  bonjour_function_arg_regno_p (REGNO)

/* Generating code for profiling - NOT IMPLEMENTED.  */
#undef  FUNCTION_PROFILER
#define FUNCTION_PROFILER(STREAM, LABELNO)      \
{                                               \
  sorry ("profiler support for BONJOUR");          \
}
...
#ifdef REG_OK_STRICT
#define BONJOUR_REG_OK_FOR_BASE_P(X)	BONJOUR_REGNO_OK_FOR_BASE_P (REGNO (X))
#define REG_MODE_OK_FOR_BASE_P(X, MODE)	\
  REGNO_MODE_OK_FOR_BASE_P (REGNO(X), MODE)
#define REG_OK_FOR_INDEX_P(X)   REGNO_OK_FOR_INDEX_P (REGNO (X))
#else /* not REG_OK_STRICT.  */
#define BONJOUR_REG_OK_FOR_BASE_P(X)	1
