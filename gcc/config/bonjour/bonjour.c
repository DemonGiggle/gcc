...
/* Current frame information calculated by bonjour_compute_frame_size.  */
static struct bonjour_frame_info current_frame_info;
...
static void bonjour_print_operand (FILE *, rtx, int);
static void bonjour_print_operand_address (FILE *, machine_mode, rtx);

/* Stack layout and calling conventions.  */
#undef  TARGET_STRUCT_VALUE_RTX
#define TARGET_STRUCT_VALUE_RTX		bonjour_struct_value_rtx
#undef  TARGET_RETURN_IN_MEMORY
#define TARGET_RETURN_IN_MEMORY		bonjour_return_in_memory

/* Target-specific uses of '__attribute__'.  */
#undef  TARGET_ATTRIBUTE_TABLE
#define TARGET_ATTRIBUTE_TABLE 		bonjour_attribute_table
#undef TARGET_NARROW_VOLATILE_BITFIELD
#define TARGET_NARROW_VOLATILE_BITFIELD hook_bool_void_false

/* EH related.  */
#undef TARGET_UNWIND_WORD_MODE
#define TARGET_UNWIND_WORD_MODE		bonjour_unwind_word_mode

/* Override Options.  */
#undef TARGET_OPTION_OVERRIDE
#define TARGET_OPTION_OVERRIDE  	bonjour_override_options 

/* Conditional register usuage.  */
#undef TARGET_CONDITIONAL_REGISTER_USAGE 
#define TARGET_CONDITIONAL_REGISTER_USAGE bonjour_conditional_register_usage

/* Controlling register spills.  */
#undef TARGET_CLASS_LIKELY_SPILLED_P
#define TARGET_CLASS_LIKELY_SPILLED_P	bonjour_class_likely_spilled_p

/* Passing function arguments.  */
#undef TARGET_FUNCTION_ARG
#define TARGET_FUNCTION_ARG 		bonjour_function_arg
#undef TARGET_FUNCTION_ARG_ADVANCE
#define TARGET_FUNCTION_ARG_ADVANCE 	bonjour_function_arg_advance
#undef TARGET_RETURN_POPS_ARGS
#define TARGET_RETURN_POPS_ARGS 	bonjour_return_pops_args

/* Initialize the GCC target structure.  */
#undef TARGET_FRAME_POINTER_REQUIRED
#define TARGET_FRAME_POINTER_REQUIRED	bonjour_frame_pointer_required
#undef TARGET_CAN_ELIMINATE
#define TARGET_CAN_ELIMINATE 		bonjour_can_eliminate
#undef TARGET_LEGITIMIZE_ADDRESS
#define TARGET_LEGITIMIZE_ADDRESS 	bonjour_legitimize_address
#undef TARGET_LEGITIMATE_CONSTANT_P
#define TARGET_LEGITIMATE_CONSTANT_P    bonjour_legitimate_constant_p
...
#define TARGET_LIBCALL_VALUE 		bonjour_libcall_value
#undef TARGET_FUNCTION_VALUE_REGNO_P
#define TARGET_FUNCTION_VALUE_REGNO_P 	bonjour_function_value_regno_p

/* printing the values.  */
#undef TARGET_PRINT_OPERAND
#define TARGET_PRINT_OPERAND 		bonjour_print_operand
#undef TARGET_PRINT_OPERAND_ADDRESS
#define TARGET_PRINT_OPERAND_ADDRESS 	bonjour_print_operand_address

/* Relative costs of operations.  */
#undef  TARGET_ADDRESS_COST
#define TARGET_ADDRESS_COST 		bonjour_address_cost
#undef TARGET_REGISTER_MOVE_COST
#define TARGET_REGISTER_MOVE_COST 	bonjour_register_move_cost
#undef TARGET_MEMORY_MOVE_COST
#define TARGET_MEMORY_MOVE_COST 	bonjour_memory_move_cost

/* Table of machine attributes.  */
static const struct attribute_spec bonjour_attribute_table[] = {
...
static bool
bonjour_return_in_memory (const_tree type, const_tree fntype ATTRIBUTE_UNUSED)
{
...
static bool
bonjour_class_likely_spilled_p (reg_class_t rclass)
{
...
static int
bonjour_return_pops_args (tree fundecl ATTRIBUTE_UNUSED,
...
bool
bonjour_is_data_model (enum data_model_type model)
{
...
static void
bonjour_override_options (void)
{
  /* Disable -fdelete-null-pointer-checks option for BONJOUR target.
  ...
  if (bonjour_data_model)
    {
      if (strcmp (bonjour_data_model, "medium") == 0)
	data_model = DM_DEFAULT;
      else if (strcmp (bonjour_data_model, "near") == 0)
	data_model = DM_NEAR;
      else if (strcmp (bonjour_data_model, "far") == 0)
	{
	  if (TARGET_BONJOURCP)
	    data_model = DM_FAR;
	  else
	    error ("data-model=far not valid for bonjourc architecture");
	}
      else
	error ("invalid data model option -mdata-model=%s", bonjour_data_model);
    }
  else
    data_model = DM_DEFAULT;
}
...
static void
bonjour_conditional_register_usage (void)
{
  if (flag_pic)
    {
...
static void
bonjour_compute_save_regs (void)
{
  unsigned int regno;
  ...
  for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
      ...
      if (fixed_regs[regno])
	{
      ...
      if (bonjour_interrupt_function_p ())
	{
  ...
  for (regno = 0; regno < FIRST_PSEUDO_REGISTER; regno++)
	...
	if (regno >= BONJOUR_FIRST_DWORD_REGISTER)
	  current_frame_info.reg_size += BONJOUR_UNITS_PER_DWORD;
	else
...
{
  if ((regno >= 0) && (regno < BONJOUR_FIRST_DWORD_REGISTER))
    return SHORT_REGS;

  if ((regno >= BONJOUR_FIRST_DWORD_REGISTER) && (regno < FIRST_PSEUDO_REGISTER))
    return LONG_REGS;
...
static rtx
bonjour_struct_value_rtx (tree fntype ATTRIBUTE_UNUSED,
                       int incoming ATTRIBUTE_UNUSED)
{
  return gen_rtx_REG (Pmode, bonjour_ret_register ());
}
...
static bool
bonjour_function_value_regno_p (const unsigned int regno)
{
  return (regno == bonjour_ret_register ());
}

/* Create an RTX representing the place where a
   library function returns a value of mode MODE.  */
static rtx
bonjour_libcall_value (machine_mode mode,
		    const_rtx func ATTRIBUTE_UNUSED)
{
  return gen_rtx_REG (mode, bonjour_ret_register ());
}
...
{
  return gen_rtx_REG (TYPE_MODE (type), bonjour_ret_register ());
}
...
/* Implements the macro FUNCTION_ARG defined in bonjour.h.  */
static rtx
bonjour_function_arg (cumulative_args_t cum_v, machine_mode mode,
...
/* Implements the macro FUNCTION_ARG_ADVANCE defined in bonjour.h.  */
static void
bonjour_function_arg_advance (cumulative_args_t cum_v, machine_mode mode,
			   const_tree type, bool named ATTRIBUTE_UNUSED)
{
  CUMULATIVE_ARGS * cum = get_cumulative_args (cum_v);
...
void
bonjour_decompose_const (rtx x, int *code, enum data_model_type *data,
		      bool treat_as_const)
{
  *code = -1;
  *data = ILLEGAL_DM;
  switch (GET_CODE (x))
      ...
      if (*code == 0)
	{
	  if (BONJOUR_TARGET_DATA_NEAR)
	    *data = DM_DEFAULT;
	  else if (BONJOUR_TARGET_DATA_MEDIUM)
	    *data = DM_FAR;
	  else if (BONJOUR_TARGET_DATA_FAR)
	    {
	      if (treat_as_const)
		/* This will be used only for printing 
		   the qualifier. This call is (may be)
		   made by bonjour_print_operand_address.  */
		*data = DM_FAR;
	      else
		/* This call is (may be) made by 
		   bonjour_legitimate_address_p.  */
		*data = ILLEGAL_DM;
	    }
	}
      return;
	...
	bonjour_decompose_const (XEXP (x, 1), code, data, treat_as_const);
      else if (GET_CODE (XEXP (x, 1)) == CONST_INT)
	bonjour_decompose_const (XEXP (x, 0), code, data, treat_as_const);
      return;
    default:
      return;
    }
...
/* Decompose Address
   ...
   as defined in enum bonjour_addrtype.  It also fills the parameter *out.
   The decomposed address can be used for two purposes.  One to 
   check if the address is valid and second to print the address
   operand.

   Following tables list valid address supported in BONJOURC/C+ architectures.
   Legend: 
   ...
   ----Code addresses----
   Branch operands:
   disp9        : BONJOUR_ABSOLUTE       (disp)
   disp17       : BONJOUR_ABSOLUTE       (disp)
   disp25       : BONJOUR_ABSOLUTE       (disp)
   RP + disp25  : BONJOUR_REGP_REL       (base, disp)

   Jump operands:
   RP           : BONJOUR_REGP_REL       (base, disp=0)
   a24          : BONJOUR_ABSOLUTE       (disp)

   ----Data addresses----
   a20          : BONJOUR_ABSOLUTE       (disp)                near (1M)
   a24          : BONJOUR_ABSOLUTE       (disp)                medium  (16M)
   R  + d20     : BONJOUR_REG_REL        (base,  disp)         near (1M+64K)
   RP + d4      : BONJOUR_REGP_REL       (base,  disp)         far  (4G)
   RP + d16     : BONJOUR_REGP_REL       (base,  disp)         far  (4G)
   RP + d20     : BONJOUR_REGP_REL       (base,  disp)         far  (4G)
   I            : *** Valid but port does not support this
   I  + a20     : *** Valid but port does not support this
   I  + RP + d14: BONJOUR_INDEX_REGP_REL (base,  index, disp)  far  (4G)
   I  + RP + d20: BONJOUR_INDEX_REGP_REL (base,  index, disp)  far  (4G)

   Decomposing Data model in case of absolute address.

   Target Option             Address type Resultant Data ref type
   ----------------------    ------------ -----------------------
   BONJOUR_TARGET_MODEL_NEAR    ABS20        DM_DEFAULT
   BONJOUR_TARGET_MODEL_NEAR    IMM20        DM_DEFAULT
   BONJOUR_TARGET_MODEL_NEAR    ABS24        Invalid
   BONJOUR_TARGET_MODEL_NEAR    IMM32        Invalid

   BONJOUR_TARGET_MODEL_MEDIUM  ABS20        DM_DEFAULT
   BONJOUR_TARGET_MODEL_MEDIUM  IMM20        DM_DEFAULT
   BONJOUR_TARGET_MODEL_MEDIUM  ABS24        DM_FAR
   BONJOUR_TARGET_MODEL_MEDIUM  IMM32        Invalid

   BONJOUR_TARGET_MODEL_FAR     ABS20        DM_DEFAULT
   BONJOUR_TARGET_MODEL_FAR     IMM20        DM_DEFAULT
   BONJOUR_TARGET_MODEL_FAR     ABS24        DM_FAR
   BONJOUR_TARGET_MODEL_FAR     IMM32        DM_FAR.  */
enum bonjour_addrtype
bonjour_decompose_address (rtx addr, struct bonjour_address *out,
			bool debug_print, bool treat_as_const)
{
  rtx base = NULL_RTX, disp = NULL_RTX, index = NULL_RTX;
  enum data_model_type data = ILLEGAL_DM;
  int code = -1;
  enum bonjour_addrtype retval = BONJOUR_INVALID;
  ...
  switch (GET_CODE (addr))
      ...
      else if (UNSIGNED_INT_FITS_N_BITS (INTVAL (disp), 24))
	{
	  if (!BONJOUR_TARGET_DATA_NEAR)
	    ...
	    return BONJOUR_INVALID;	/* ABS24 is not support in NEAR model.  */
	}
      else
	return BONJOUR_INVALID;
      break;
    ...
    case CONST:
	 ...
	 result of assembly-time arithmetic computation.  */
      ...
      bonjour_decompose_const (XEXP (addr, 0), &code, &data, treat_as_const);
      ...
      if (code == 0)
	{
	  if (BONJOUR_TARGET_DATA_NEAR)
		...
		/* This will be used only for printing the 
		   ...
		   by bonjour_print_operand_address.  */
		data = DM_FAR;
	      else
		/* This call is (may be) made by 
		   bonjour_legitimate_address_p.  */
	  ...
	  switch (GET_CODE (XEXP (addr, 1)))
	    ...
	    case CONST:
	      /* This is also a valid expression for address.
	         ...
	         Till then fall through BONJOUR_INVALID.  */
	    default:
	      return BONJOUR_INVALID;
	    }
  ...
  if (base && !(bonjour_addr_reg_p (base)))
    return BONJOUR_INVALID;
  if (base && !(BONJOUR_REG_OK_FOR_BASE_P (base)))
    return BONJOUR_INVALID;
...
static bool
bonjour_legitimate_address_p (machine_mode mode ATTRIBUTE_UNUSED,
			   rtx addr, bool strict)
{
  enum bonjour_addrtype addrtype;
  struct bonjour_address address;
  ...
  addrtype = bonjour_decompose_address (addr, &address,
  ...
  if (TARGET_DEBUG_ADDR)
    {
      ...
      switch (addrtype)
	{
	case BONJOUR_INVALID:
	  typestr = "invalid";
	  break;
	case BONJOUR_ABSOLUTE:
	  typestr = "absolute";
	  break;
	case BONJOUR_REG_REL:
	  typestr = "register relative";
	  break;
	case BONJOUR_REGP_REL:
	  typestr = "register pair relative";
	  break;
	case BONJOUR_INDEX_REGP_REL:
	  typestr = "index + register pair relative";
	  break;
	default:
	  gcc_unreachable ();
	}
      fprintf (stderr, "\nbonjour address type: %s\n", typestr);
    }

  if (addrtype == BONJOUR_INVALID)
    return FALSE;
  ...
  if (flag_pic)
    {
      if (addrtype == BONJOUR_REG_REL || addrtype == BONJOUR_REGP_REL)
	return TRUE;
      else
	return FALSE;
    }
...
static int
bonjour_address_cost (rtx addr, machine_mode mode ATTRIBUTE_UNUSED,
		   ...
		   bool speed ATTRIBUTE_UNUSED)
{
  ...
  addrtype = bonjour_decompose_address (addr, &address, 0, FALSE);

  gcc_assert (addrtype != BONJOUR_INVALID);

  /* BONJOUR_ABSOLUTE            : 3
     BONJOUR_REG_REL  (disp !=0) : 4
     BONJOUR_REG_REL  (disp ==0) : 5
     BONJOUR_REGP_REL (disp !=0) : 6
     BONJOUR_REGP_REL (disp ==0) : 7
     BONJOUR_INDEX_REGP_REL (disp !=0) : 8
     BONJOUR_INDEX_REGP_REL (disp ==0) : 9.  */
  switch (addrtype)
    {
    case BONJOUR_ABSOLUTE:
      cost += 1;
...
static int
bonjour_register_move_cost (machine_mode mode ATTRIBUTE_UNUSED,
...
/* Return the cost of moving data of mode MODE between a register of class
   ...
   REGISTER_MOVE_COST.  */
static int
bonjour_memory_move_cost (machine_mode mode,
...
/* Check if a const_double is ok for bonjour store-immediate instructions.  */
int
bonjour_const_double_ok (rtx op)
{
...
int
bonjour_operand_bit_pos (int val, int bitval)
{
  int i;
  if (bitval == 0)
    val = ~val;
...
/* Implements the macro PRINT_OPERAND defined in bonjour.h.  */
static void
bonjour_print_operand (FILE * file, rtx x, int code)
{
  ...
  switch (code)
    ...
    case 'd':
      {
	const char *bonjour_cmp_str;
	switch (GET_CODE (x))
	  {
	    /* MD: compare (reg, reg or imm) but BONJOUR: cmp (reg or imm, reg)
	...
	fprintf (file, "%s", bonjour_cmp_str);
	return;
      }
    case '$':
      putc ('$', file);
    ...
    case 'r':
      ...
      fprintf (file, "%d", bonjour_operand_bit_pos (INTVAL (x), 0));
      return;

    case 's':
      /* Print bit position of first 1.  */
      fprintf (file, "%d", bonjour_operand_bit_pos (INTVAL (x), 1));
      return;
    case 'g':
	...
	default:
	  ...
	  bonjour_print_operand_address (file, VOIDmode, x);
	  return;
	}
      gcc_unreachable ();
    default:
...
/* Implements the macro PRINT_OPERAND_ADDRESS defined in bonjour.h.  */

static void
bonjour_print_operand_address (FILE * file, machine_mode /*mode*/, rtx addr)
{
  enum bonjour_addrtype addrtype;
  struct bonjour_address address;

  /* Decompose the address. Also ask it to treat address as constant.  */
  addrtype = bonjour_decompose_address (addr, &address, 0, TRUE);
  ...
  switch (addrtype)
      ...
      if (address.disp)
	{
	  if (GET_CODE (address.disp) == UNSPEC)
	    bonjour_print_operand (file, address.disp, 0);
	  else
  ...
  if (flag_pic < NEAR_PIC && address.code == 0)
    ...
    {
      /* REVISIT: bonjour register indirect jump expects a 1-bit right shifted
...
/* Called from bonjour.md. The return value depends on the parameter push_or_pop:
...
char *
bonjour_prepare_push_pop_string (int push_or_pop)
{
  ...
  while (i <= current_frame_info.last_reg_to_save)
	     ...
	     && (i <= current_frame_info.last_reg_to_save))
	{
	  ...
	  if (i == RETURN_ADDRESS_REGNUM)
	  ...
	  else
	    {
	      ...
	      if ((word_cnt + ((i < BONJOUR_FIRST_DWORD_REGISTER) ? 1 : 2))
		  >= MAX_COUNT)
		break;
	      /* Increase word count by 2 for long registers except RA.   */
	      word_cnt += ((i < BONJOUR_FIRST_DWORD_REGISTER) ? 1 : 2);
	    }
	  ++i;
	}
	...
	{
	  /* Pop instruction.  */
	  if (print_ra && !bonjour_interrupt_function_p ()
...
static void
bonjour_create_dwarf_for_multi_push (rtx insn)
{
  rtx dwarf, reg, tmp;
  ...
  for (i = 0; i <= current_frame_info.last_reg_to_save; ++i)
	...
	{
	  ++num_regs;
	  if (i < BONJOUR_FIRST_DWORD_REGISTER)
	    total_push_bytes += 2;
	  else
	    total_push_bytes += 4;
	}
  ...
  for (i = current_frame_info.last_reg_to_save; i >= 0;)
      ...
      if (!current_frame_info.save_regs[i] || 0 == i || split_here)
	{
	  ...
	  for (j = to; j >= from; --j)
	    {
	      if (j < BONJOUR_FIRST_DWORD_REGISTER)
		{
		  mode = HImode;
		  inc = 1;
		}
      ...
      if (i != RETURN_ADDRESS_REGNUM)
	{
	  inc = (i < BONJOUR_FIRST_DWORD_REGISTER) ? 1 : 2;
...
/*
CompactRISC BONJOUR Architecture stack layout:
  ...
  if (current_frame_info.last_reg_to_save != -1)
    {
      ...
      bonjour_create_dwarf_for_multi_push (insn);
...
static bool
bonjour_frame_pointer_required (void)
{
...
static bool
bonjour_can_eliminate (const int from ATTRIBUTE_UNUSED, const int to)
{
...
static rtx
bonjour_legitimize_address (rtx x, rtx orig_x ATTRIBUTE_UNUSED,
...
/* Implement TARGET_LEGITIMATE_CONSTANT_P
   ...
   satisfies CONSTANT_P. In bonjourc treat legitimize float 
   constant as an immediate operand.  */
static bool
bonjour_legitimate_constant_p (machine_mode mode ATTRIBUTE_UNUSED,
...
/* Helper function for md file. This function is used to emit arithmetic 
   ...
   printed.  */
const char *
bonjour_emit_add_sub_di (rtx *operands, enum rtx_code code)
{
  rtx lo_op[2] ;
  rtx hi0_op[2] ;
  rtx hi1_op[2] ;
...
/* Helper function for md file. This function is used to emit logical 
   ...
   printed.  */
const char *
bonjour_emit_logical_di (rtx *operands, enum rtx_code code)
{
  rtx lo_op[2] ;
  rtx hi_op[2] ;
